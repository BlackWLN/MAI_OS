...
void parallelBitonicSort(int *arr, int n, int dir) {
    ...
    while (N < n) N <<= 1;
    char buffer[100];
    snprintf(buffer, sizeof(buffer), "Initial size: %d, After size: %d\n", n, N);
...
void testWithSmallArray() {
    ...
    int test_size = 4;
    write(STDOUT_FILENO, "Initial massive: ", 18);
    ...
    write(STDOUT_FILENO, "Sorted massive: ", 17);
    for (int i = 0; i < test_size; i++) {
        char num[12];
        snprintf(num, sizeof(num), "%d ", test_arr[i]);
        write(STDOUT_FILENO, num, strlen(num));
    ...
    if (isSorted(test_arr, test_size)) {
        write(STDOUT_FILENO, "Test OK\n", 9);
    } else {
        write(STDOUT_FILENO, "Test FAILED\n", 13);
    }
}
...
int main(int argc, char *argv[]) {
    if (argc != 3) {
        ...
        snprintf(usage_msg, sizeof(usage_msg), 
                "Usage: %s <num_of_thr> <size_of_massive>\n", 
                argv[0]);
        write(STDOUT_FILENO, usage_msg, strlen(usage_msg));
        write(STDOUT_FILENO, "Example: ", 10);
    ...
    char info[100];
    snprintf(info, sizeof(info), "Sort %d, threads: %d\n", n, MAX_THREADS);
    write(STDOUT_FILENO, info, strlen(info));
    snprintf(info, sizeof(info), "PID: %d\n", getpid());
    write(STDOUT_FILENO, info, strlen(info));
    write(STDOUT_FILENO, "First 10: ", 11);
    ...
    char time_msg[50];
    snprintf(time_msg, sizeof(time_msg), "Time: %.4f \n", time_taken);
    ...
    write(STDOUT_FILENO, "\n", 1);
    if (isSorted(arr, n)) {
        write(STDOUT_FILENO, "Massive have been sorted\n", 26);
    } else {
        write(STDOUT_FILENO, "ERR!\n", 6);
    }
    ...
    for (int threads = 1; threads <= 8; threads *= 2) {
        ...
        for (int s = 0; s < num_sizes; s++) {
            ...
            snprintf(result, sizeof(result), 
                    "Size: %d, Time: %.4f s, Correct: %s\n", 
                    n, time_taken, sorted ? "Yes" : "No");
